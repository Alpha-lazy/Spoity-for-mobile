...
function Artist() {
  ...
  const {spngId,setSongId,songId,song,setSong} = useData();
  const naviagte = useNavigate()

  // get all info about artist using artistId

  const get_artist_data = async() => {
      try {
        setLoader(true);
        const responce  = await axios.request({
          url:`https://jiosavan-api2.vercel.app/api/artists/${artistId}`,
          method:"GET",   
          params:{
            page:0,
            songCount:10,
          ...
          setArtist(responce.data.data);
          getAverageColor(responce.data.data.image[2].url).then((color)=>{
            document.querySelector('.artist-main-container').style.background = `linear-gradient(${color.rgb}, transparent 80%)`
          });
       setLoader(false);
  ...
  async function getAverageColor(imageUrl, ratio = 0.1) {
    ...
    setSongData(responce.data.data[0])
  ...
  return (
        ...
        :
       <>
        <div className='artist-main-container' style={{width:"100%",height:"100vh",display:"flex",flexDirection:"column",background:"linear-gradient(rgb(141, 74, 44), transparent 80%)"}}>
        <header style={{width:"100%",padding:"10px 15px",position:"sticky",top:"0px"}} >
            <svg onClick={()=>{naviagte(-1)}} data-encore-id="icon" role="img" aria-hidden="true" viewBox="0 0 24 24" style={{width: "23px", height: "23px", fill: "rgb(255, 255, 255)"}}><path d="M13.414 3.5a.999.999 0 0 0-1.707-.707l-9.207 9.2 9.207 9.202a1 1 0 1 0 1.414-1.413L6.335 13H20.5a1 1 0 0 0 0-2H6.322l6.799-6.794a.999.999 0 0 0 .293-.707z"></path></svg>
        </header>
        <div style={{width:"100%",display:"flex",flexDirection:"column",gap:"10px"}}>

            <div className='artist-image playlist-image' style={{display:"flex",alighnItems:"center",justifyContent:"center",width:"100%",padding:"15px"}}>
                 
                <div style={{aspectRatio:"1/1",borderRadius:"50%",overflow:"hidden",filter: "drop-shadow(0 0 18.3529px rgba(0, 0, 0, .3))"}}>
                        <img src={artist?.image[2].url}  style={{width:"100%",height:"100%",objectFit:"cover",objectPosition:"center center"}}/>
                </div>

      
            </div>
                <div className="artist_Info" style={{display:"flex",flexDirection:"column",gap:"5px",padding:"0px 15px"}}>
                     <div className='artist_name' style={{fontSize:"25px",fontWeight:"700",color:"#fff"}}>
                     ...
                     <div className='artist_listners' style={{fontSize:"13px",fontWeight:"500",color:"#fff"}}>
                           {artist?.fanCount}  fan count
                     </div>

                     
                </div>

                <div style={{marginTop:"15px",display:"flex",justifyContent:"space-between",padding:"0px 15px"}}>
                  <div style={{display:"flex",gap:"10px",alignItems:'center'}}>

                      <button style={{width:"auto",padding:"10px 20px",color:"white",backgroundColor:"transparent",border:"1px solid white",fontSize:"12px",borderRadius:"50px"}}>
                          Follow
                      </button>
            ...
            >
              ...
              >
                ...
                <div
                  ...
                  onClick={()=>{
                    songData.artists.primary.forEach(element => {
                      if (element.id != artistId) {
                        return naviagte(`/artist/${element.id}`)
                      }
